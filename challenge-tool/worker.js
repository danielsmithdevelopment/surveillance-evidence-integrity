 ...
 *   2. Accuracy (FRE 702 / Daubert) — ~10% OCR misreads; LAPD OIG 32.3% hot-list false positives
...
const VENDORS = {
  flock: {
    ...
    errorRateFacts: [
      ...
      "In his August 13, 2026 blog post, Flock CEO Garrett Langley claimed 'fewer than 9 human-reported errors for every 1 million alerts.' This claim measures optical character recognition errors — misreads of plate characters. The LAPD Inspector General's July 10, 2026 official government audit measured a different and more legally relevant metric: false positive actionable hot-list alerts that triggered armed high-risk stops of innocent people. The LAPD found 161 false positives out of 498 alerts — 32.3%. Flock's 9-per-million figure and the LAPD's 32.3% are not contradictory because they measure different things. Flock is measuring whether the camera correctly read the plate. The LAPD is measuring whether the alert caused an armed stop of an innocent person. For FRE 702 Daubert purposes, the relevant metric is the one that produces wrongful enforcement actions — which is 32.3%, not 0.0009%. The Flock CEO's framing conflates these two metrics in a way that materially misrepresents the system's reliability as an enforcement tool. flocksafety.com/blog/flock-guardrails-address-lpr-privacy-concerns-and-police-transparency, August 13, 2026.",
    ],
    accessAbuseFacts: [
      ...
      "On August 13, 2026, Flock CEO Garrett Langley announced that Audit Assistance — which detects abnormal activity and flags it for administrator review — would become mandatory for all law enforcement customers by end of year after only 'more than one-third' of customers had voluntarily adopted it. Langley stated that in recent weeks Audit Assistance reviews 'have been associated with arrests of several law enforcement officers who allegedly abused the system.' That statement is an admission that (a) Flock's own monitoring identified abuse resulting in arrests, and (b) roughly two-thirds of customers — including departments where documented stalking cases occurred — operated without mandatory abuse detection during the period of those cases. Langley also announced mandatory case codes for all law enforcement searches by end of year (optional since July 2025), proactive lockout on abnormal behavior, and offense filtering for cross-agency sharing. Flock Safety blog, August 13, 2026.",
    ],
    authFacts: [
      ...
      "On August 13, 2026, Flock Safety CEO Garrett Langley published a blog post announcing a series of changes in direct response to documented abuse cases and accuracy failures. The post constitutes Flock's admission that: (1) the prior voluntary Audit Assistance adoption rate of one-third of customers was insufficient and mandatory adoption is now required; (2) the prior voluntary case code requirement was insufficient and mandatory case codes are now required for all law enforcement searches; (3) the prior thirty-day default data retention period is being reduced to seven days; (4) prior contractual language 'confused the public' about data ownership — a direct reference to the Eagle Sports Range situation. Each of these mandatory changes is an implicit admission that the prior voluntary system was inadequate during the period when the documented stalking, abuse, and unauthorized access cases occurred. Langley also announced mandatory MFA (as of early August 2026), proactive lockout on abnormal behavior, offense filtering for sharing, a Bishop Fox independent security review with findings to be published in September 2026, and a forthcoming plain-English contract explanation. Flock Safety blog, August 13, 2026 — flocksafety.com/blog/flock-guardrails-address-lpr-privacy-concerns-and-police-transparency.",
    ],
    civilFacts: [
    ...
    sources: [
      ...
      "Flock Safety / Garrett Langley — Guardrails blog (Aug. 13, 2026): mandatory Audit Assistance, case codes, 7-day retention, MFA",
...
On August 13, 2026, Flock CEO Garrett Langley published a blog post announcing mandatory case codes, mandatory Audit Assistance, proactive lockout, MFA, and reduced default retention (30 days → 7 days) — each described as a new or newly mandatory requirement. These mandatory requirements are admissions that the prior voluntary system was inadequate. For cases arising from the period before these mandatory controls were implemented, the authentication and access-accountability argument is strengthened by Flock's own admission that adequate accountability controls did not exist (or were not required). The prior system — under which the documented stalking, unauthorized access, Condor exposure, and abuse cases occurred — is the system whose outputs are now being offered as evidence. Langley's statement that prior contractual language "confused the public" about data ownership further supports the Eagle Sports Range / public-ownership-claim contradiction. Demand discovery of when Audit Assistance and case codes were enabled for the agency at issue; whether they were voluntary or mandatory on the incident date; and any Audit Assistance flags related to the querying officer.
...
In response to accuracy criticism including the LAPD Inspector General's July 2026 audit finding a 32.3% false positive rate on actionable hot-list alerts, Flock CEO Garrett Langley claimed on August 13, 2026 that the system produces "fewer than 9 human-reported errors for every 1 million alerts." This claim measures optical character recognition plate-read errors, not false positive actionable alerts. The LAPD audit measured false positive alerts that triggered armed high-risk stops of innocent people — a 32.3% rate. These measure different things. For FRE 702 purposes, the relevant reliability metric is the rate at which the system produces false enforcement actions against innocent people, not whether the camera correctly photographed a plate. The Flock CEO's conflation of these two metrics in public communications is itself relevant to the Daubert hearing — it demonstrates that Flock's public accuracy claims obscure the system's actual enforcement-action error rate. Cite the Langley blog and the LAPD OIG audit side by side; demand discovery of how Flock defines and counts "human-reported errors" versus hot-list false positives that result in stops.

### Vector 3 — Access abuse and Fourth Amendment
ALPR data used to initiate a stop or investigation is a search. Argue on three prongs when facts fit: (1) unauthorized / undocumented access (28+ personal-use cases; 84% FOIA queries without case numbers); (2) concealment and pretext (404 Media August 2026 series); (3) the ALPR dragnet itself as a warrantless general search under the Carpenter → Smith → Reeves tower-dump line. Discovery into officer query history and warrant authorization is essential. Flock's August 13, 2026 announcement that case codes and Audit Assistance will become mandatory (after only one-third voluntary Audit Assistance adoption, and after Audit Assistance reviews were associated with officer arrests) is an admission that the voluntary accountability regime under which documented abuse occurred was inadequate — cite for cases whose searches predate mandatory rollout.
...
4. The error rate problem: (a) approximately 10% plate character misread rate (2019 estimate, DHS 2025 market survey); (b) at least 27 documented wrongful stops and detentions from ALPR errors since 2018 (Institute for Justice); (c) Oak Park oversight board 2025 finding that Flock cameras played no meaningful role in any crime investigation during three years of deployment; and (d) when ${vendorName} is Flock Safety or hot-list facts apply — the LAPD Office of the Inspector General audit released July 10, 2026, finding a 32.3% false positive rate on Flock hot-list alerts (161 of 498) during August–September 2025, each false alert resulting in a traffic stop of an innocent driver under high-risk stop protocol (backup, air support, supervisor; driver ordered out). Emphasize the Red Banyan / reporting nuance that strengthens Daubert: false positives were driven by stale hot-list database records, not OCR misreads — so the relevant reliability unit is cameras + database + alert mechanism. Quote the Inspector General's liability language. Note LAPD allowed its three-year contract to expire rather than renew. (e) When Flock: contrast CEO Garrett Langley's August 13, 2026 claim of "fewer than 9 human-reported errors for every 1 million alerts" (OCR / human-reported plate-read errors) with the LAPD OIG's 32.3% false-positive rate on actionable hot-list alerts that triggered stops — argue the CEO conflates two different metrics and that FRE 702 cares about wrongful enforcement actions, not OCR alone.
